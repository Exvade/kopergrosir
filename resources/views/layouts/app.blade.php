    ...
    <nav class="bg-white/90 backdrop-blur-md sticky top-0 z-50 border-b border-blue-50" x-data="{ open: false }">
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="px-6 py-8 space-y-4">
                <a href="#home" @click="open = false" class="block text-xl font-bold text-primary hover:text-secondary transition">Home</a>
                <a href="#about" @click="open = false" class="block text-xl font-bold text-primary hover:text-secondary transition">Tentang Kami</a>
                <a href="#paket" @click="open = false" class="block text-xl font-bold text-primary hover:text-secondary transition">Paket Bundling</a>
                <a href="#katalog" @click="open = false" class="block text-xl font-bold text-primary hover:text-secondary transition">Katalog</a>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
                ...
                <div class="flex flex-col items-start space-y-5">
                    ...
                    <p class="text-slate-500 text-xs leading-relaxed max-w-[220px]">
                        Pusat distribusi perlengkapan travel, Haji, dan Umroh terpercaya dengan standar industrial terbaik.
                    </p>
                </div>

                <div>
                    <h4 class="font-bold text-primary mb-6 uppercase text-[10px] tracking-[0.2em] border-l-2 border-secondary pl-3">Navigasi</h4>
                    <ul class="space-y-3">
                        <li><a href="#home" class="text-sm text-slate-500 hover:text-secondary transition">Beranda</a></li>
                        <li><a href="#about" class="text-sm text-slate-500 hover:text-secondary transition">Tentang Kami</a></li>
                        <li><a href="#paket" class="text-sm text-slate-500 hover:text-secondary transition">Paket Bundling</a></li>
                        <li><a href="#katalog" class="text-sm text-slate-500 hover:text-secondary transition">Katalog Produk</a></li>
                        <li><a href="#testimoni" class="text-sm text-slate-500 hover:text-secondary transition">Testimoni</a></li>
                    </ul>
                </div>

                <div>
                    <h4 class="font-bold text-primary mb-6 uppercase text-[10px] tracking-[0.2em] border-l-2 border-secondary pl-3">Koleksi Utama</h4>
                    <ul class="space-y-3">
                        @foreach ($categories as $cat)
                            <li><a href="#katalog" class="text-sm text-slate-500 hover:text-secondary transition">{{ $cat->name }}</a></li>
                        @endforeach
                        <li><a href="#paket" class="text-sm font-bold text-secondary underline decoration-blue-200 underline-offset-4 hover:text-primary transition">Paket Distributor</a></li>
                    </ul>
                </div>

                <div>
                    <h4 class="font-bold text-primary mb-6 uppercase text-[10px] tracking-[0.2em] border-l-2 border-secondary pl-3">Hubungi Kami</h4>
                    <div class="space-y-4">
                        <div class="flex items-start gap-3">
                            <svg class="w-4 h-4 text-secondary mt-1 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-xs text-slate-500 font-semibold">+{{ $waNumber->value ?? '' }}</span>
                        </div>
                    </div>
                </div>
            </div>

            <div class="pt-8 border-t border-blue-50 flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-left">
